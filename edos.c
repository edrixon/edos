...
#define MEMPOOL_SIZE     128     // Number of memory blocks in pool
...
typedef enum { HIGH, LOW } logicLevel;
...
void ledOnProcess()
{
    ...
    {
        ledTrigger = false;
        bspGpioWrite(LED_BUILTIN, HIGH);
        strcpy(msg, "msg: LED ON");
        edosSendMsg(&ledMsgQueue, msg);
    }
}
...
void ledMgmtProcess()
{
    ...
    if(edosReadMsg(&ledMsgQueue, msg) == true)
    {
        bspPrintln(msg);
    }
}
